...
void EspLuaEngine::_loadLibraries() {
  // Стандартные библиотеки Lua отключены от сборки (lbaselib/lstrlib/ltablib/
  // lmathlib/lutf8lib и ldump переименованы в *.c.disabled). Движок «игрушечный»:
  // в сценариях доступны только сам язык Lua и функции, которые регистрирует
  // module_macros (puts/print/clock/now/cron/cond/term/button).
}
